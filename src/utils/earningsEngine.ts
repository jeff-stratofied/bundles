...
// feeThisMonth in amort row is computed on full loan balance — scale by ownership here
const feeThisMonth = row.isOwned && scale > 0
  ? +Number((row.feeThisMonth || 0) * scale).toFixed(2)
  : 0
...
if (row.isOwned && !deferred) {
  ...
  feesThisMonth = feeThisMonth        // ← now correctly scaled
}
